...
{
	//UE_LOG(LogTemp, Display, TEXT("INTERACT ho raha hai !")); 
	...
	DrawDebugSphere(GetWorld(), End, InteractionSphereRadius, 20, FColor::Blue, false, 10.0f);
	...
	if (HasHit)
	{
		...
		else if (HitActor->ActorHasTag("Lock"))
		{
			...
			if (LockActor)
			{
				...
				{
					//2- KeyItemName has to be present in our inventory(ItemList)
				...
				else
				{
					...
					//here lock has a key inside, now we remove that
					//Deactivate the lock
